...
async def _init_tables(pool) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            """
            CREATE TABLE IF NOT EXISTS generator_email_domains (
...
async def load_accounts() -> Optional[list]:
    """
    DatabaseLoadAkunKonfigurasi（Jika）
    ...
    try:
        ...
        if data:
            logger.info(f"[STORAGE] DatabaseLoad {len(data)} Akun")
        else:
            logger.info("[STORAGE] DatabasebelumAkun")

        return data
    except Exception as e:
        logger.error(f"[STORAGE] DatabaseBacaGagal: {e}")
    return None
...
# ========== Generator Email Domains ==========

async def add_generator_domain(domain: str) -> bool:
    """Tambah domain generator.email ke database"""
    if not is_database_enabled():
        return False
    backend = _get_backend()
    try:
        if backend == "postgres":
            ...
            async with pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO generator_email_domains (domain, is_active) VALUES ($1, TRUE) ON CONFLICT (domain) DO NOTHING",
                    domain.lower().strip(),
                )
            return True
        if backend == "sqlite":
            ...
            with _sqlite_lock, conn:
                conn.execute(
                    "INSERT OR IGNORE INTO generator_email_domains (domain, is_active) VALUES (?, 1)",
                    (domain.lower().strip(),),
                )
            return True
    except Exception as e:
        logger.error(f"[STORAGE] Add generator domain failed: {e}")
    return False


async def remove_generator_domain(domain: str) -> bool:
    """Hapus domain generator.email dari database"""
    if not is_database_enabled():
        return False
    backend = _get_backend()
    try:
        if backend == "postgres":
            ...
            async with pool.acquire() as conn:
                await conn.execute(
                    "DELETE FROM generator_email_domains WHERE domain = $1",
                    domain.lower().strip(),
                )
            return True
        if backend == "sqlite":
            ...
            with _sqlite_lock, conn:
                conn.execute(
                    "DELETE FROM generator_email_domains WHERE domain = ?",
                    (domain.lower().strip(),),
                )
            return True
    except Exception as e:
        logger.error(f"[STORAGE] Remove generator domain failed: {e}")
    return False


async def toggle_generator_domain(domain: str, is_active: bool) -> bool:
    """Toggle aktif/nonaktif domain"""
    if not is_database_enabled():
        return False
    backend = _get_backend()
    try:
        if backend == "postgres":
            ...
            async with pool.acquire() as conn:
                await conn.execute(
                    "UPDATE generator_email_domains SET is_active = $1 WHERE domain = $2",
                    is_active,
                    domain.lower().strip(),
                )
            return True
        if backend == "sqlite":
            ...
            with _sqlite_lock, conn:
                conn.execute(
                    "UPDATE generator_email_domains SET is_active = ? WHERE domain = ?",
                    (1 if is_active else 0, domain.lower().strip()),
                )
            return True
    except Exception as e:
        logger.error(f"[STORAGE] Toggle generator domain failed: {e}")
    return False
...
async def get_generator_domains(active_only: bool = True) -> list:
    ...
    try:
        if backend == "postgres":
            ...
            async with pool.acquire() as conn:
                if active_only:
                    rows = await conn.fetch("SELECT domain FROM generator_email_domains WHERE is_active = TRUE ORDER BY id")
                else:
                    rows = await conn.fetch("SELECT domain FROM generator_email_domains ORDER BY id")
        ...
        if backend == "sqlite":
            ...
            with _sqlite_lock:
                if active_only:
                    rows = conn.execute("SELECT domain FROM generator_email_domains WHERE is_active = 1 ORDER BY id").fetchall()
                else:
                    rows = conn.execute("SELECT domain FROM generator_email_domains ORDER BY id").fetchall()
            return [row["domain"] for row in rows]
    except Exception as e:
...
def add_generator_domain_sync(domain: str) -> bool:
    """Sync version - tambah domain"""
    return _run_in_db_loop(add_generator_domain(domain))


def remove_generator_domain_sync(domain: str) -> bool:
    """Sync version - hapus domain"""
    return _run_in_db_loop(remove_generator_domain(domain))


def toggle_generator_domain_sync(domain: str, is_active: bool) -> bool:
    """Sync version - toggle domain"""
    return _run_in_db_loop(toggle_generator_domain(domain, is_active))


def get_generator_domains_sync(active_only: bool = True) -> list:
    """Sync version - ambil list domain"""
    return _run_in_db_loop(get_generator_domains(active_only))
