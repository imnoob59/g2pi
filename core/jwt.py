...
class JWTManager:
    ...
    async def _refresh(self, request_id: str = "") -> None:
        ...
        if r.status_code != 200:
            logger.error(f"[AUTH] [{self.config.account_id}] {req_tag}JWT Gagal: {r.status_code}")
        ...
        logger.info(f"[AUTH] [{self.config.account_id}] {req_tag}JWT Berhasil")
